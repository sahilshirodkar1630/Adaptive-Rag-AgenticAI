...
- Prefix private methods and module-level state with an underscore (e.g. `_faiss_vectorstore`, `_private_method()`)
...
> **Project note:** several existing modules (`graph_tools.py`, `retriever_setup.py`, `document_upload.py`) currently use `print()` for status/debug output (e.g. `print(f"[doc_tool] score={score}, rewrite_count={rewrite_count}")`). New code should use `logger.debug()` / `logger.info()` instead; migrating the existing `print()` calls is tracked as cleanup work rather than a blocker.
...
- ✓ Tests pass
- ✓ Any behavior change to loop-guard counters (`rewrite_count`, `verify_count`) is reflected in `README.md`, `DOCUMENT_UPLOAD_FLOW.md`, and `DOCUMENT_FLOW_VISUAL.md` where relevant
...
- Log intermediate steps for debugging (grading score, rewrite/verify counters, routing decisions)
- **Loop guards are load-bearing**: `doc_tool()` caps `rewrite_count` at 3 before falling back to `web_search`, and `route_after_verify()` caps `verify_count` at 2 before returning the best available answer. If you change either threshold, update it consistently in `graph_tools.py`, the docstrings, and `README.md`.
- **Vector store writes are currently destructive**: `retriever_chain()` rebuilds the FAISS store from only the newly uploaded chunks — it does not merge with existing content. Don't assume prior uploads are still queryable; see `DOCUMENT_UPLOAD_FLOW.md` before changing this behavior.
